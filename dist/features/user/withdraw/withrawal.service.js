...
const withdrawRequest_response_1 = require("../../../shared/types/interfaces/responses/user/withdrawRequest.response");
...
class WithdrawalService {
    constructor({ userModel, transactionModel, withdrawalRequestModel, encryptionRepo }) {
        ...
        this.enterBankName = () => __awaiter(this, void 0, void 0, function* () {
            return `CON Enter Bank Name`;
        });
        this.withdraw = (phoneNumber, amount, accountNumber, bank) => __awaiter(this, void 0, void 0, function* () {
            const checkUser = yield this._userModel.checkIfExist({ phoneNumber });
            if (!checkUser.data)
                return `END Unable to get your account`;
            ...
            const burnMsg = yield this.beepTxClient.burn((parseFloat(amount) * 1000000).toString());
            ...
            if (!burnBeepToken.status)
                return `END Unable to perform transaction`;
            const reference = this.generateUniqueCode();
            const newTransaction = yield this._transactionModel.createTransactionToDB({ userId: id, amount: parseFloat(amount), reference: reference, type: transaction_response_1.TransactionTypeEnum.DEBIT, status: transaction_response_1.TransactionStatus.PENDING });
            if (!newTransaction.data)
                return `END Unable to create transaction`;
            const newWithdrawalRequest = yield this._withdrawalRequestModel.createWithrawalRequestToDB({ userId: id, amount: parseFloat(amount), bank: bank, accountNumber: accountNumber, reference: reference, status: withdrawRequest_response_1.WithdrawalRequestStatus.PENDING });
            if (!newWithdrawalRequest.data)
                return `END Unable to create transaction`;
